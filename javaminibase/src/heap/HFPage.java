...
public class HFPage extends Page 
  ...
  */
  ...
  private int getpctotal() {
	  return pc.counter;
  }
  
  public void printPageUtil() {
	  try {
		int used_space = data.length - available_space();
		System.out.println("Page utilization of current page is " + (float)used_space/data.length);
	} catch (IOException e) {
		e.printStackTrace();
	}
