...
public class Sort1 extends Iterator implements GlobalConst {
	...
	public Sort(AttrType[] in, short len_in, short[] str_sizes, Iterator am,
		...
		this(in, len_in, str_sizes, am, sort_fld, sort_order, sort_fld_len, n_pages, false);
	}

	public Sort(AttrType[] in, short len_in, short[] str_sizes, Iterator am,
			int sort_fld, TupleOrder sort_order, int sort_fld_len, int n_pages,
			boolean b) throws IOException, SortException {
		...
		if(b){
			Q = new pnodeSplayPQ(sort_fld,new AttrType( AttrType.attrReal), order);
		}else{
	...
	public Tuple get_next() throws IOException, SortException, UnknowAttrType,
		...
		if (first_time) {
			...
			if(sortOnScore){
				// generate runs
				Nruns = generate_runs(max_elems_in_heap,new AttrType( AttrType.attrReal),
						sortFldLen);
