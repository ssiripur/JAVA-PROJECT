...
public class TopRankJoin3 extends Iterator {
	...
	public TopRankJoin3 (int numTables,AttrType[][] in,int[] len_in,short[][] s_sizes,int[] join_col_in,
	...
	// out[0]=new CondExpr[numTables];
	 ...
	 System.out.println(relnames[i]);
		...
		tempTuple.setHdr((short) 4,in[tIndex], s_sizes[tIndex]);
        ...
        int joinSize=4;
			...
			{
				tempTuple.print(in[tIndex]);
			 // tempTuple = am[tIndex].get_next();
				//tempTuple.setHdr((short) 4,in[tIndex], s_sizes[tIndex]); 
			  //System.out.println("temp tuple:"+ tempTuple);
			  joinkey_AttrType = in[tIndex][join_col_in[tIndex]-1];
			
			 if (joinkey_AttrType.attrType == AttrType.attrInteger) 
			 	{		
				 //System.out.println("join int value"+tempTuple.getIntFld(join_col_in[tIndex]));
			 	...
			 	{
				  //System.out.println("join string value"+tempTuple.getStrFld(join_col_in[tIndex]));
...
//						}
//                  }
				joinkey_ssize = joinkey_Value.toString().length();
			...
			if (keyIndex == -1) 
				{
				  ...
				  System.out.println("temp score: "+tempTuple.getScore());
	...
	public Tuple get_next()
		    throws JoinsException, IndexException, InvalidTupleSizeException, InvalidTypeException, PageNotReadException, TupleUtilsException, PredEvalException, SortException, LowMemException, UnknowAttrType, UnknownKeyTypeException, Exception
		    {
		     ...
		     catch(Exception e)
		     {
		    	 ...
		    	 System.out.println("ir value "+ir1);
		     }
		      Object o=null;
		      ...
		      if(at[0].attrType==AttrType.attrString)
		      {
		    	...
		    	System.out.println(o.toString());
		    	  ...
		    	  {  e.printStackTrace();
		    		  System.out.println("Exception at TopRankJoin3 near getStrFld");
		    	  }
		      }
		   ...
		   //  FldSpec[] proj_list=new FldSpec[];
		      for(int i=0;i<numTables1;i++)
		      {
		    	 ...
		    	 IndexScan is=new IndexScan(index1[i],indNames1[i],
		                "BTreeIndex",in1[i],s_sizes[i],len_in[i],1,proj_list,out[i],join_col_in[i],true); 
		    	 tuple[i]=is.get_next();
			...
			//  s=null;
			 ...
			 for(int i=0;i<joinKey_Objects.size();i++)
			 {
				...
				// Object o=j.keyval;
				 ...
				 if(joinkey_AttrType.attrType==AttrType.attrString)
					{
					 System.out.println("key value: "+j.keyval);
