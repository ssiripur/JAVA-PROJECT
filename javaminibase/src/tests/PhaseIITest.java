...
public class PhaseIITest implements GlobalConst{
	...
	public static void processTopNestedLoopJoin(){
			...
			for(int k=0;k<numOfTables;k++)
			{
				...
				try 
				{
				    ...
				    if(k==0){
						...
						try {
							am = new IndexScan(b_index, tableNameList[0], tableNameList[0]+"_BTreeIndex", attrTypeList[0],
									stringSizesList[0], (short)numOfColsList[k], (short)numOfColsList[k], Sprojection, null, joinedColList[0], false);
						}
		...
		try {
			nlj = new TopNestedLoopsJoins(attrTypeList[0], numOfColsList[0], stringSizesList[0], attrTypeList[1], numOfColsList[1],
					stringSizesList[1], memory, am, tableNameList[1], condExprList, null, projList, projlistIndex, topK);
		} catch (Exception e) {
		...
		try {
			...
			sort_names = new Sort(JJtype, (short) projlistIndex, stringSizesList[0],(iterator.Iterator) nlj, 1, ascending, jjSize, 5);
	...
	public static void processTopSortMerge(){
...
//						/System.out.println("projlistIndex="+projlistIndex); 
						projList[projlistIndex] = new FldSpec(rel, colNum);
						colNumList[projColTableSpecificOffset -1]=colNum;
		...
		for(int k=0;k<numOfTables;k++){
			...
			try {
				...
				for(int y=0;y<projTableSpecificList[k].length;y++){
					System.out.println("projTableSpecificList[k].length ="+projTableSpecificList[k].length);
					System.out.println("projTableSpecificList[k][y].relation.key ="+projTableSpecificList[k][y].relation.key);
				}
				System.out.println("No of out fields ="+projTableSpecificList[k].length);
				System.out.println("Attr type size = "+attrTypeList[k].length);
			  	iscan = new IndexScan(new IndexType(IndexType.B_Index), indexNameList[k]+".in", indexNameList[k], attrTypeList[k], stringSizesList[k], numOfColsList[k],projTableSpecificList[k].length/*projlistIndex*/, 
			  			projTableSpecificList[k]/*projList*/, condExprList, joinedColList[k], true);
			  	iteratorList[k] = iscan;
			}
			catch (Exception e) {
				status = false;
			    e.printStackTrace();
	...
	public static void main(String[] args) {
		...
		try{
			...
			switch(option){
				...
				case 1:  processTopNestedLoopJoin();
					break;
			
				case 2: processTopSortMerge();
					break;
